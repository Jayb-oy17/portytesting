...
import { Github, Star, GitFork, Code2, Activity, ExternalLink, MessageSquare, X, Send } from 'lucide-react';
...
function ChatWidget() {
  ...
  const [messages, setMessages] = useState<{ from: 'user' | 'bot'; text: string }[]>([
    { from: 'bot', text: "Hi! I'm your GitHub activity assistant. Ask me anything about this developer's work!" },
  ]);
  ...
  const respond = (q: string) => {
    ...
    let answer = "I'm not sure about that. Try asking about projects, languages, or recent activity!";
  ...
  const send = (text?: string) => {
    ...
    if (!q) return;
    setMessages(prev => [...prev, { from: 'user', text: q }, { from: 'bot', text: respond(q) }]);
    setInput('');
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {open && (
        <div className="mb-3 w-80 rounded-2xl border border-sky-500/30 bg-card shadow-2xl shadow-sky-500/10 overflow-hidden flex flex-col" style={{ height: 380 }}>
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-sky-500 to-teal-500">
            <div className="flex items-center gap-2">
              <MessageSquare className="w-4 h-4 text-white" />
              <span className="text-white font-semibold text-sm">GitHub Assistant</span>
            </div>
          ...
          {messages.length <= 2 && (
            <div className="px-3 py-1 flex flex-wrap gap-1">
              {PRESET_QS.map((q) => (
                <button key={q} onClick={() => send(q)} className="text-xs px-2 py-0.5 rounded-full border border-sky-500/30 text-sky-500 hover:bg-sky-500/10 transition-colors">
                  {q}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-1 p-2 border-t border-border/50">
            ...
            />
            <button onClick={() => send()} className="p-2 rounded-lg bg-gradient-to-r from-sky-500 to-teal-500 text-white hover:opacity-90 transition-opacity">
  ...
  return (
    <div className="pt-16">
      ...
      <section className="py-20 bg-gradient-to-b from-sky-500/5 to-transparent border-b border-border/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              <span className="text-sm font-semibold text-sky-500 uppercase tracking-widest">Open Source</span>
              <h1 className="text-4xl md:text-5xl font-bold mt-2 mb-2">
                GitHub <span className="gradient-text">Activity</span>
              </h1>
              <p className="text-muted-foreground">@{profile.githubUsername} · 37+ public repositories</p>
            </div>
            <a
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <h2 className="text-2xl font-bold mb-8">Language <span className="gradient-text">Breakdown</span></h2>
          </FadeIn>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <FadeIn key={l.lang} delay={i * 100}>
                <div className="rounded-2xl border border-border bg-card p-5 hover:border-sky-500/30 transition-all">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="w-3 h-3 rounded-full" style={{ background: langColors[l.lang] || '#888' }} />
                    <span className="font-semibold text-sm">{l.lang}</span>
                  </div>
                  <div className="text-3xl font-bold gradient-text mb-1">{l.count}</div>
                  <div className="text-xs text-muted-foreground mb-2">repositories</div>
                  ...
                  <div className="text-xs text-muted-foreground mt-1">{l.pct}%</div>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      <section className="py-16 bg-muted/20 border-t border-border/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <h2 className="text-2xl font-bold mb-8">Featured <span className="gradient-text">Repositories</span></h2>
          </FadeIn>
                ...
                <div className="rounded-xl border border-border bg-card p-4 hover:border-sky-500/30 transition-all group">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-sm group-hover:text-sky-500 transition-colors">{p.title}</span>
                    </div>
                    <div className="flex gap-1.5">
                      {p.repoUrl && (
                        <a href={p.repoUrl} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-sky-500 transition-colors">
                          <Github className="w-3.5 h-3.5" />
                        </a>
                      )}
                      {p.liveUrl && (
                        <a href={p.liveUrl} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-teal-500 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full" style={{ background: langColors[p.language] || '#888' }} />
